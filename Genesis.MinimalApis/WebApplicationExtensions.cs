...
public static class WebApplicationExtensions {
    ...
    static WebApplication RegisterAttributes(WebApplication app, object? instance, Type type) {
        ...
        foreach(var (action, routeTemplate, verb) in results)
            ...
            // typeof(EndpointRouteBuilderExtensions).GetMethod(
            //     $"Map{verb.First() + verb[1..].ToLower()}", 0, 
            //     new Type[] { typeof(IEndpointRouteBuilder), typeof(string), typeof(Delegate) }
            // )?.Invoke(null, new object[] { app, routeTemplate, action });
